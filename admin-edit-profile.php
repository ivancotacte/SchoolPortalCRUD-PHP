...
if (isset($_POST['submit'])) {
    ...
    if ($password !== $confirmPassword) {
        ...
        $sql = "UPDATE TBL_ACCOUNT SET IMAGE = ?, FIRST_NAME = ?, MIDDLE_NAME = ?, LAST_NAME = ?, SUFFIX_NAME = ?, COURSE = ?, CAMPUS = ?, CONTACT_NUMBER = ?, EMAIL_ADDRESS = ?, PASSWORD = ? WHERE STUDENT_NUMBER = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$image, $firstName, $middleName, $lastName, $suffixName, $course, $campus, $contactNumber, $email, $hashedPassword, $_SESSION['STUDENT_NUMBER']]);

        $msg = "<div class='alert alert-success' role='alert'>Profile updated successfully.</div>";
        header("Location: owner-dashboard.php");
    } else {
        $sql = "UPDATE TBL_ACCOUNT SET IMAGE = ?, FIRST_NAME = ?, MIDDLE_NAME = ?, LAST_NAME = ?, SUFFIX_NAME = ?, COURSE = ?, CAMPUS = ?, CONTACT_NUMBER = ?, EMAIL_ADDRESS = ? WHERE STUDENT_NUMBER = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$image, $firstName, $middleName, $lastName, $suffixName, $course, $campus, $contactNumber, $email, $_SESSION['STUDENT_NUMBER']]);
